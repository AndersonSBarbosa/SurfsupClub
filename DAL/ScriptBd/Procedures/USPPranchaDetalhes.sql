PRINT Db_Name()+', Criação da Procedure USPPranchaDetalhes'

GO
--------------------------------------------------------------------------------------
-- OBJETO: USPPranchaDetalhes – RESPONSÁVEL: Anderson – DATA: 10/10/2018 – 
--------------------------------------------------------------------------------------
IF EXISTS( SELECT
               NAME
           FROM
           ...
           WHERE
             XTYPE = 'P'
             AND NAME = 'USPPranchaDetalhes' )
    DROP PROCEDURE USPPranchaDetalhes

GO

CREATE PROCEDURE USPPranchaDetalhes
		   @IDPRANCHA int
AS
    SET NOCOUNT ON
...
WHERE TB_PRANCHA.IDPRANCHA = @IDPRANCHA