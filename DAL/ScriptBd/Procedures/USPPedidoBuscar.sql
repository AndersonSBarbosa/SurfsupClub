PRINT Db_Name()+', Criação da Procedure USPPedidoBuscar'

GO
--------------------------------------------------------------------------------------
-- OBJETO: USPPedidoBuscar – RESPONSÁVEL: Anderson – DATA: 10/10/2018 – 
--------------------------------------------------------------------------------------
IF EXISTS( SELECT
               NAME
           FROM
           ...
           WHERE
             XTYPE = 'P'
             AND NAME = 'USPPedidoBuscar' )
    DROP PROCEDURE USPPedidoBuscar

GO

CREATE PROCEDURE USPPedidoBuscar
		   @IDPEDIDO INT
AS
    SET NOCOUNT ON
