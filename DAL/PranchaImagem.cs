   ...
   public class PranchaImagem
    {
        ...
        public void create(Entidades.PranchaImagem Obj)
        {
            ...
            {
                int Resposta = 0;
                string sql = @"USPPranchaImagemCadastro";
                ...
                using (RaiCore.Data conn = new RaiCore.Data())
                {
                    ...
                    using (DbCommand cmd = conn.CreateCommand())
                    {
                        ...
                        conn.AddInParameter(cmd, "@IMG", DbType.String, Obj.Img);
        ...
        public void delete(Entidades.PranchaImagem obj)
        {
            try
            {
                string sql = @"USPPranchaImagemExcluir";
                using (RaiCore.Data conn = new RaiCore.Data())
                {
                    conn.RaiConnection("SurfsUpClubConnection");
            ...
            try
            {
                using (RaiCore.Data conn = new RaiCore.Data())
                {
                    ...
                    using (DbCommand cmd = conn.CreateCommand())
                    {
                        ...
                        conn.AddInParameter(cmd, "@IDPRANCHA", DbType.Int32, id);
                        //conn.AddInParameter(cmd, "@IDUSUARIO", DbType.Int32, IdUsuario);
        ...
        public bool find(Entidades.PranchaImagem Obj)
        {
            ...
            try
            {
                ...
                if (TemRegistros)
                {
                    ...
                    Obj.IdStatus = Convert.ToInt32(reader[4].ToString());
                    Obj.Status.NomeStatus = reader[5].ToString();
                    if (!String.IsNullOrEmpty(reader[6].ToString()))
                    {
                        Obj.DataCadastro = DateTime.Parse(reader[6].ToString());
                    }
                    Obj.Img = (reader[7].ToString());
                    Obj.Capa = Convert.ToBoolean(reader[8].ToString());
